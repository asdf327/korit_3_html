...
- box-show : (inset여부) offset-x offset-y (blur-radius) (spread-radius) 
...
- 종류 
  ...
  - absolut
    - top : 30px left 30px; 라고 가정했을 때, 상의 위치가 다르다는 점을 확인 가능
    - 의 속성 값을 첫 부모 요소를 기준으로 설정 
    - 이 속성값은 자리를 차지하지 않는다 
    - 그래서 디폴트 위치인 상태에서 한 지정한다면 회색 정사각형이 사라졌다고 생각할 수 있지만 사실 겹쳐져 보이던 것이다 

    - 이후 오렌지를 빨간색을 이라고 가정하면 갑자기 빨간색 상자가 정신나간 위치로 이동한다 

    - 는 이상의 상황을 보았을 때 _첫 부모 요소가 이 아닌 것을 기준_ 으로 한다 
    ...
    - static
      - 어떤 요소가 스크롤로 이동할 수 있는 공간을  속성값을 이용하여 지정함 
      - 웹사이트 상단 메뉴바가 어느 지점을 지나면 특정 위치에 고정되는 것을 볼 수 있는데 이때 사용하는 편

      - 부모 요소의 여백 등에는 영향을 받지 않음. 지식의 요소의 위치를 고정해놓더라도 부모 요소의 위치를 벗어난다면 자식 요소는 그 이상을 움직이지 않음
      - 이상의 이우로 static의 경우는 부모 
...
- 겹쳐지는 순서를 인위적으로 바꾸려고 할 때  속성을 사용
- 디폴트는 auto == 0.
- 숫자가 높을수록 높은 위치에 있음 
...
옛날에는 태그와 태그만으로 노가다 형태로 사이트 구조를 만드는 경우들이 많았지만 
